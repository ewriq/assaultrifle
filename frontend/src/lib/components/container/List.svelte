<script lang="ts">
  ...
  import { fetchAllContainers, type Container } from "$lib/api/container/list";
  ...
  <ul class="space-y-2">
    {#each containers as c}
      <li class="border p-3 rounded bg-white shadow">
        <strong><a class="text-blue-400" href="/p/{c.Token}">{c.Name}</a></strong> — Port: {c.Port} — Tür: {c.Types}
      </li>
    {/each}
  </ul>
{/if}
