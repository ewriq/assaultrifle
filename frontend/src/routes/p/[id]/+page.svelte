...
<main class="p-6 max-w-3xl mx-auto">
  ...
  <div class="flex gap-2 my-4">
    <button
      ...
      class="bg-green-500 px-3 py-1 text-white rounded">Başlat</button
    >
    <button
      on:click={() => handleAction("stop")}
      class="bg-yellow-500 px-3 py-1 text-white rounded">Durdur</button
    >
    <button
      on:click={() => handleAction("restart")}
      class="bg-blue-500 px-3 py-1 text-white rounded">Yeniden Başlat</button
    >
    <button
      on:click={() => handleAction("delete")}
      class="bg-red-600 px-3 py-1 text-white rounded">Sil</button
    >
  </div>
  ...
  <pre
    class="bg-gray-100 p-4 rounded mt-2 overflow-x-auto"
    style="white-space: pre-wrap;">{logs}</pre>
